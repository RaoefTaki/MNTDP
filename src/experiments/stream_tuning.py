...
def train_on_tasks(config):
    ...
    for t_id, (task, vis_p) in enumerate(zip(tasks, task_vis_params)):
        ...
        if task_level_tuning:
            ...
            nr_of_architectures = 7 if len(learner.connections) == 1 else 12
            ...
            _, _, _, _, nr_of_parameters_saved_list = \
                get_bw_transfer_info(tasks=tasks, last_task_id=t_id, task_vis_params=task_vis_params, config=config,
                                     learner=learner, tasks_list=tasks_list, tasks_bw_output_head=tasks_bw_output_head)
    ...
    keys_list, values_list, old_evaluation_accuracy_list, new_evaluation_accuracy_list, nr_of_parameters_saved_list =\
        get_bw_transfer_info(tasks=tasks, last_task_id=None, task_vis_params=task_vis_params, config=config,
                             learner=learner, tasks_list=tasks_list, tasks_bw_output_head=tasks_bw_output_head)
...
def check_possibility_backward_transfer(memory_buffer=None, task_id=None, task=None, tasks_list=None, learner=None,
    ...
    for p_t_id in range(task_id):
        ...
        # no found samples
        if not p_t_labels.issubset(c_t_labels) or not p_t_samples:
            continue
...
def train_t(config):
    ...
    tune.utils.wait_for_gpu(target_util=1-(1/division_factor_per_gpu), retry=200)  # Try 10 times as much
