...
def train_on_tasks(config):
    ...
    for t_id, (task, vis_p) in enumerate(zip(tasks, task_vis_params)):
        ...
        if task_level_tuning:
            ...
            value_actor_remote_initial = ray.get(value_actor.get.remote())
            print(value_actor_remote_initial)
            value_actor.set.remote(12)
            value_actor_remote_second = ray.get(value_actor.get.remote())
...
def train_t(config):
    ...
    raise ValueError("Test if crashes before the actor is used fully")
