...
def train_on_tasks(config):
    ...
    for t_id, (task, vis_p) in enumerate(zip(tasks, task_vis_params)):
        ...
        if task_level_tuning:
            ...
            config['optim'] = [{'architecture': {'grid_search': list(range(1))}, 'lr': {'grid_search': [0.01]}, 'weight_decay': {'grid_search': [0]}}]  # TODO: REMOVE
            ...
            ray_params['resources_per_trial'] = {'cpu': 0.5, 'gpu': 0}  # TODO: 0.25  # TODO: somehow this changes per run if set dynamically?
...
def train_t(config):
    ...
    # tune.utils.wait_for_gpu(target_util=1-(1/division_factor_per_gpu))  # TODO: reenable
